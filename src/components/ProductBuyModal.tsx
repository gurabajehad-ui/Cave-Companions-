...
export const ProductBuyModal: React.FC<ProductBuyModalProps> = ({
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      setError(err.message || 'কার্টে পণ্য যোগ করতে ব্যর্থ হয়েছে।');
    } finally {
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/70">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900 leading-tight">পণ্য নির্বাচন ও টোকেন ডিসকাউন্ট</h3>
                <p className="text-xs text-slate-500 font-medium">{shop.nameBn || shop.name}</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="p-6 overflow-y-auto space-y-5">
            {addedSuccess ? (
              <div className="py-6 text-center space-y-4">
                ...
                <div>
                  <h4 className="text-lg font-bold text-slate-900">পণ্যটি কার্টে সফলভাবে যুক্ত হয়েছে!</h4>
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-3 justify-center">
                  ...
                  <button
                    ...
                    className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl text-sm transition flex items-center justify-center gap-2 shadow-sm"
                  >
                    <ShoppingCart className="w-4 h-4" />
                    কার্ট দেখুন ও অর্ডার করুন
                  </button>
                </div>
              </div>
            ) : (
              <>
                ...
                <div className="flex gap-4 p-3.5 bg-slate-50 rounded-xl border border-slate-200/80 items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="inline-block px-2 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-200 text-[10px] font-bold rounded mb-1">
                      {product.isAvailable ? 'উপলব্ধ (In Stock)' : 'অনুপলব্ধ'}
                    </span>
                    ...
                    <div className="text-sm font-extrabold text-slate-900 mt-1">
                      মূল্য: ৳{Number(originalPrice || 0).toLocaleString('bn-BD')}
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider">
                    টোকেন ডিসকাউন্ট প্রয়োগ করুন (ঐচ্ছিক)
                  </label>

                  {loading ? (
                    <div className="p-3 text-center text-xs text-slate-400">টোকেন চেক করা হচ্ছে...</div>
                  ) : tokens.length === 0 ? (
                    <div className="p-3 bg-amber-50 rounded-xl border border-amber-200/60 text-xs text-amber-800 flex items-start gap-2">
                      ...
                      <span>আপনার কাছে বর্তমানে কোনো অব্যবহৃত রিওয়ার্ড টোকেন নেই। রেগুলার মূল্যে অর্ডার করতে পারেন।</span>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      {/* No token option */}
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <p className="text-xs font-bold text-slate-800">কোনো টোকেন ব্যবহার করব না</p>
                            <p className="text-[11px] text-slate-500">রেগুলার মূল্য প্রযোজ্য হবে</p>
                          </div>
                        </div>
                        <span className="text-xs font-bold text-slate-700">৳{originalPrice}</span>
                      </button>
                              ...
                              <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                  ...
                                  <p className="text-[11px] text-emerald-700 font-medium">
                                    {token.isDonated 
                                      ? 'এই টোকেনের ডিসকাউন্টটি সরাসরি কল্যাণ তহবিলে দান হবে' 
                                      : `ছাড়: ৳${calcUnitDiscount.toFixed(0)} / ইউনিট`
                                    }
                                  </p>
                                </div>
                              </div>
                              <div className="text-right">
                                <span className="text-xs font-extrabold text-emerald-700">৳{calcFinalPrice.toFixed(0)}</span>
                                <span className="text-[10px] text-slate-400 line-through block">৳{originalPrice}</span>
                              </div>
                            </div>
                            {token.isDonated && (
                ...
                <div className="p-3.5 bg-slate-900 text-white rounded-xl space-y-1.5 text-xs">
                  <div className="flex justify-between text-slate-300">
                    <span>পণ্যের রেগুলার মূল্য:</span>
                    <span>৳{Number(totalOriginal || 0).toLocaleString('bn-BD')}</span>
                  </div>
                  {totalDiscount > 0 && (
                    <div className="flex justify-between text-emerald-400 font-semibold">
                      <span>টোকেন ডিসকাউন্ট ({discountRate}%):</span>
                      <span>- ৳{Number(totalDiscount || 0).toLocaleString('bn-BD')}</span>
                    </div>
                  )}
                  {totalDonationAmount > 0 && (
                    <div className="flex justify-between text-rose-400 font-semibold">
                      <span>কল্যাণ তহবিলে দানকৃত ({normalDiscountRate}%):</span>
                      <span>৳{Number(totalDonationAmount || 0).toLocaleString('bn-BD')} ❤️</span>
                    </div>
                  )}
                  <div className="border-t border-slate-800 pt-1.5 flex justify-between text-sm font-bold text-white">
                    <span>প্রদেয় পণ্যের মূল্য:</span>
                    <span className="text-emerald-400 font-extrabold">৳{Number(totalPayable || 0).toLocaleString('bn-BD')}</span>
                  </div>
                  <p className="text-[10px] text-slate-400 pt-1">
                    * চেকআউট করার সময় স্ট্যান্ডার্ড ডেলিভারি চার্জ (৳৬০) যোগ হবে।
                  </p>
                </div>
                ...
                >
                  {addingToCart ? (
                    <span className="animate-pulse">কার্টে যোগ হচ্ছে...</span>
                  ) : (
                    <>
                      <ShoppingCart className="w-4 h-4" />
                      কার্টে যোগ করুন
