...
const endMarker = "      {/* PENDING MOSQUE REVIEW MODAL */}";
...
// I want to replace from startMarker to the end of the mosqSubTab === 'pending' block.
// The block ends right before </div> } for the main Tab View.
// Let's just do a regex replace if possible, but actually we can just find:
// `            </div>\n          )}\n        </div>\n      )}` 
// and replace from startMarker to that.

const searchString = "          {/* PENDING MOSQUES SECTION */}";
const endString = "          )}\n        </div>\n      )}";
const blockEndIdx = code.indexOf(endString, startIdx);
...
code = code.slice(0, startIdx) + replacement + code.slice(blockEndIdx + endString.length);
fs.writeFileSync('src/components/AdminDashboardView.tsx', code);
console.log("Successfully replaced pending section 2!");
